...
| Cove S (mobile wave-powered small-AUV service station) | [cove-s/MODEL.md](cove-s/MODEL.md) |
...
Rebuild the five generated browser pages (sheet, product, node, tube, Cove S) from the repo root with `node build.mjs` — the strip page (`strip/simulation.html`) is hand-authored and not templated. Regenerate the parameter sweeps in `RESULTS.md` with `node run.mjs`.
