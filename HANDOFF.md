...
What is here: five energy-balanced screening models, one per folder (`strip/model.mjs`, `sheet/sheet.mjs`, `product/full-model.mjs`, `tube/tube-model.mjs`, `node/node-model.mjs`) alongside their test scripts, plus `build.mjs` (assembles offline HTML pages), `run.mjs` (writes `RESULTS.md`), `MODEL.md` (equations and limits). Do not edit the built `*-simulation.html` directly; edit the templates or models and run `node build.mjs`.
